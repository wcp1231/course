...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\h a -> if f h then h :. a else a) Nil
...
flatten ::
  List (List a)
  -> List a
flatten l = foldRight (\a b -> a ++ b) Nil l
...
find ::
  ...
  -> List a
  -> Optional a
find f = let listToOpt Nil = Empty
             listToOpt (a :. _) = Full a
           in listToOpt . filter f
